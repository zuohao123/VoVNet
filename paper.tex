...
The policy is trained jointly with an explicit accuracy--cost objective using
counterfactual loss targets and optional gain supervision, yielding controllable
Pareto trade-offs.
...
reliably backpropagate into the policy in our common straight-through setting.
We therefore train the policy with an explicit supervised signal derived from
counterfactual losses under \texttt{no\_grad}. For each sample we compute
...
and add small biases that penalize \no\ early in training to avoid collapse.
...
policy indirectly through the counterfactual losses used in
\cref{eq:policy_target}. We optionally mix $\pi$ with a small uniform
exploration probability $\epsilon$ to ensure the policy sees all branches during
training. The soft-mixture path exists but is disabled when image-token
...
enforce a minimum full-action ratio during warmup to reduce collapse.
...
When profiling latency/memory, we explicitly control KV-cache reuse settings and
report both token-cost proxies and their empirical correlation with wall-clock
latency to make cost proxies auditable.
...
Operationally, composition is defined as:
...
This analysis clarifies the cost/benefit trade-off of gain supervision. In our
implementation, policy targets already require counterfactual losses; gain
...
time/step plus tokens/s, which can be computed directly from training logs.
...
measured latencies, and we include a correlation analysis between token-cost
proxies and wall-clock latency/memory to validate that the cost proxy is a
useful deployment surrogate.
