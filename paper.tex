...
policy training (up to 40k steps). Stage~2 uses gain supervision with a ranking hinge loss
($\text{margin}=0.2$), entropy regularization, and a small exploration probability ($\epsilon=0.05$).
We linearly warm up $\lambda_{\text{cost}}$ over the first 6k steps of Stage~2 and sweep
$\lambda_{\text{cost}}$ to trace an accuracy--cost Pareto curve. We report 2--3 random seeds for
selected $\lambda_{\text{cost}}$ values.
...
\begin{itemize}
  ...
  \item \textbf{Random Policy (ratio-matched):} samples actions to match \method's action ratios (global) or
  entropy-bucketed ratios (low/mid/high), testing whether gains come from per-instance decisions.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.30\columnwidth}cccc}
    ...
    Res. Scaling (best) & 0.510 & 3.673 & 0/0/100 & 2238.6 \\
    Token Pruning (best) & \textit{TBD} & \textit{TBD} & 0/0/100 & \textit{TBD} \\
    Token Merge (best) & 0.510 & 1.193 & 0/0/100 & 2473.5 \\
    Multi-Gran. (best) & 0.510 & 1.302 & 0/0/100 & 1814.6 \\
...
Stage~2 trains the policy head for up to 40k steps. We use a ranking hinge gain loss
($\text{margin}=0.2$) with weight $\lambda_{\text{gain}}=6.0$, entropy regularization
($\lambda_{\text{ent}}=0.005$), and a small exploration probability $\epsilon=0.05$.
We linearly warm up $\lambda_{\text{cost}}$ over the first 6k steps of Stage~2 and sweep
$\lambda_{\text{cost}} \in \{0, 0.01, 0.02, 0.05, 0.1\}$ for Pareto curves.
...
on V100 GPUs. Checkpoints are saved every 2{,}000 steps.
