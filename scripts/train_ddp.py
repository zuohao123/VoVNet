                        ...
                        if actions is None and train_mode == "soft_mixture":
                            ...
                            if actions is None and cfg.policy.policy_target_mode != "none":
                                ...
                                if loss_triplet is not None:
                                    if cfg.policy.policy_target_mode == "loss_cost_soft":
                                        token_count_coarse = outputs.get("token_count_coarse")
                                        token_count_full = outputs.get("token_count_full")
                                        ...
                                        if cfg.policy.cost_mode == "fixed":
                                            zeros = token_count_coarse.new_zeros(token_count_coarse.shape)
                                            cost_c = token_count_coarse.new_full(
                                                token_count_coarse.shape, float(cfg.policy.cost_c1)
                                            )
                                            cost_f = token_count_coarse.new_full(
                                                token_count_coarse.shape, float(cfg.policy.cost_c2)
                                            )
                                        ...
                                        combined = loss_triplet + cost_triplet * cost_scale * float(lambda_cost)
                                        temp = float(cfg.policy.soft_target_temperature)
                                        policy_targets_soft = torch.softmax(-combined / temp, dim=-1)
                                        policy_targets_hard = combined.argmin(dim=-1)
                                    elif cfg.policy.policy_target_mode == "loss_margin":
                                        token_count_coarse = outputs.get("token_count_coarse")
                                        token_count_full = outputs.get("token_count_full")
                                        if (
                                        ...
                                        if (
                                            ...
                                            if force_steps > 0 and (stage_steps + 1) <= force_steps:
                                                ...
                                                if diff_open.numel() == 1:
                                                    ...
                                                    threshold = float(
                                                        torch.quantile(
                                                            diff_open.detach(), open_quantile_step
                                                        ).item()
                                                    )
                                        ...
                                        if (
                                            ...
                                            and cfg.policy.policy_min_full_warmup_steps > 0
                                        ):
                                        ...
                                        if cfg.policy.enable_soft_targets and policy_targets_hard is not None:
                                            temp = float(cfg.policy.soft_target_temperature)
                                            policy_targets_soft = torch.softmax(-loss_triplet / temp, dim=-1)
                                            if (
